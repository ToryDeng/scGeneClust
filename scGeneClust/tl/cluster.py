...
def clustering(
    ...
    res = Parallel(n_jobs=-1)(delayed(mutual_info_regression)
    ...
    node_pairs = Parallel(n_jobs=-1)(delayed(prune)(adata, node1, node2, w['weight'], scale) 
...
def prune(adata, node1, node2, weight, scale):
    ...
    complm = complementarity(adata, node1, node2, 42)
