...
#' 
#' param md: a data frame of V-measure values of clustering results.
#' param fl: the title of the color bar of rankings.
#' return a ggplot object.
hmap <- function(md,fl){
  ## generate the heat map in the composite plot
...
#' methods in cell clustering on 12 scRNA-seq datasets. 
#' 
#' param mdnum: a data frame of V-measure values of clustering results.
#' param marg: a vector of the plot margins.
#' return a ggplot object.
box_plot_method_md <-function(mdnum,marg){
  ## generate the box plot in the composite plot
  p <- ggplot(mdnum, aes(x=method,y=rank))+
    ...
    theme(axis.text.y=element_blank(),
          ...
          axis.title.y=element_blank(),
          axis.ticks.y=element_blank(),
          #legend.text=element_markdown(size=13,colour="black"),
...
#' Using the combination of heat map and box plot and bar plot to perform the 
#' comparison of GeneClust with competing FS methods in cell clustering on 12 
#' scRNA-seq datasets. The methods are ranked by V-measure values of the
#' clustering results.
#' 
#' param method_input: a character represents the clustering method.
#' return two ggplot objects.
comparison_V_measure <- function(method_input){
  ## choose data sheet 
...
#generate figures####
suppressWarnings(ggsave(filename = paste0("Figure S4 B",".png"),
                        comparison_V_measure("seurat")$ar2000,dpi = 300,
                        height = 6,width = 9))
suppressWarnings(ggsave(filename = paste0("Figure S4 A",".png"),
                        comparison_V_measure("seurat")$com,dpi = 300,height = 8,
                        width = 14))
suppressWarnings(ggsave(filename = paste0("Figure S6 B",".png"),
                        comparison_V_measure("TSCAN")$ar2000,dpi = 300,
                        height = 6,width = 9))
suppressWarnings(ggsave(filename = paste0("Figure S6 A",".png"),
                        comparison_V_measure("TSCAN")$com,dpi = 300,height = 8,
                        width = 14))
suppressWarnings(ggsave(filename = paste0("Figure S5 B",".png"),
                        comparison_V_measure("SC3s")$ar2000,dpi = 300,
                        height = 6,width = 9))
suppressWarnings(ggsave(filename = paste0("Figure S5 A",".png"),
                        comparison_V_measure("SC3s")$com,dpi = 300,height = 8,
                        width = 14))
suppressWarnings(ggsave(filename = paste0("Figure S7 B",".png"),
                        comparison_V_measure("KMeans")$ar2000,dpi = 300,
                        height = 6,width = 9))
suppressWarnings(ggsave(filename = paste0("Figure S7 A",".png"),
                        comparison_V_measure("KMeans")$com,dpi = 300,height = 8,
                        width = 14))
