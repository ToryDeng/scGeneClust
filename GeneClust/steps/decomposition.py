...
def reduce_dimension(
    ...
    if cache is not None:
        ...
        if dr_method == 'pca':
            embedding = sc.tl.pca(adata.T.X, n_comps=n_componets)  # compute PCs on normalized data
        elif dr_method == 'glm-pca':
            embedding = glmpca(adata.raw.X, L=n_componets, ctl={"maxIter": 500, "eps": 1e-3})[
                "factors"]  # use raw counts
        elif dr_method == 'umap':
