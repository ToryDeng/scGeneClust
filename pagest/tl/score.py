...
def score_discriminative_gene(adata: ad.AnnData, stat: Literal['kw', 'f'], use_rep: str = 'log-normalized'):
    ...
    if stat == 'f':
        ...
        s = np.sqrt(np.sum([np.sum((X - X[y == cluster].mean(0)) ** 2, axis=0) for cluster in clusters], axis=0) /
                    (n_samples - n_clusters))
        # difference for all clusters
        d = np.vstack([(X[y == cluster].mean(0) - all_mean) /
                       (np.sqrt(1 / (y == cluster).sum() - 1 / n_samples) * (s + np.median(s)))  # m_k * (s + s_0)
                       for cluster in clusters])
        # threshholding and compute variance
        adata.var['stat'] = np.var(np.where(d < delta, 0, d), axis=0)
    elif stat == 'deviance':
        X, y = adata.layers['X_cell_ori'], adata.obs['cluster']
        pi = X.sum(0) / X.sum()
        cluster_centroid = np.vstack([X[y == cluster].mean(0) for cluster in np.unique(y)])
        n = cluster_centroid.sum(1)[:, np.newaxis]

        with np.errstate(all='ignore'):
            left_half = np.nansum(cluster_centroid * np.log(cluster_centroid / n.dot(pi[np.newaxis, :])), axis=0)
            right_half = np.nansum((n - cluster_centroid) * (np.log((n - cluster_centroid) / (n.dot((1 - pi)[np.newaxis, :])))), axis=0)
        adata.var['stat'] = 2 * (left_half + right_half)
    else:
